...
def atualizar_dados_agente(jogadasAgenteI, bancoDeDados, ganhador, ganhou, perdeu, empatou):

    # Atualiza os valores de ranked em função do resultado
    for registro in jogadasAgenteI:

        if(ganhador == 1):  # Agente perdeu
            registro["ranked"][registro["jogada"]] -= perdeu

        elif(ganhador == 2):  # Agente ganhou
            registro["ranked"][registro["jogada"]] -= ganhou

        else:  # Empate
            registro["ranked"][registro["jogada"]] += empatou

    # Atualiza o banco de dados com os registros atualizados de ranked
    for IA in jogadasAgenteI:
        ...
        # Busca no banco de dados o tabuleiro correspondente
        for registro in bancoDeDados:

            if(registro["tabuleiro"] == tabuleiro_IA):

                registro["ranked"] = ranked_IA  # Atualiza o ranked existente
                encontrado = True

                break  # Sai do loop ao encontrar o tabuleiro correspondente

        # Adiciona o novo tabuleiro se não foi encontrado no banco de dados
        if(not encontrado):
            bancoDeDados.append({"tabuleiro": tabuleiro_IA, "ranked": ranked_IA})
...
with open('banco_conhecimento2.txt', 'r') as arquivo:
    bancoDados2 = json.load(arquivo)
...
while True:
        ...
        if (modoSimulacao == 1):
            ...
            while quantRodadas < quantPartidas:
                ...
                if (len(jogada) > 8 or ganhador != 0):
                    ...
                    for registro in jogadasAgenteI1:
            ...
            while quantRodadas < quantPartidas:
                ...
                if (len(jogada) > 8 or ganhador != 0):
                    ...
                    # Utilização da função para os dois agentes
                    atualizar_dados_agente(jogadasAgenteI1, bancoDados, ganhador, ganhou, perdeu, empatou)
                    atualizar_dados_agente(jogadasAgenteI2, bancoDados2, ganhador, ganhou, perdeu, empatou)

                    print("\n\nAgnete Inteligente 1: ", jogadasAgenteI1)
                    print("\n\nAgnete Inteligente 2: ", jogadasAgenteI2)
                    quantRodadas += 1
                    limparTabuleiro(tabuleiro)
                    jogadasAgenteI1.clear()
                    jogadasAgenteI2.clear()
                    jogada.clear()  # Limpa a lista de jogadas
                    ganhador = 0
    ...
    if (salvar == "S"):
        ...
        elif (modoJogo == 5 and modoSimulacao == 3):
            arquivo_excel = 'resultado_AgenteI_VS_AgenteI_MSM_Banco.xlsx'
            jogador1 = 'Agente Inteligente 1'
            jogador2 = 'Agente Inteligente 2'
    ...
    if (salvarBanco == "S"):
        ...
        with open('banco_conhecimento2.txt', 'w') as arquivo:
            json.dump(bancoDados2, arquivo)
